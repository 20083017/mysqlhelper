...
def process(worker_id, datasource):
    ...
    while True:
        ...
        if status:
            ...
            #     # # 查询单条
            # SELECT ID,UID,FromName,MessageType,SendTime,s_msgid2 FROM TopicMsgNew WHERE gid=5031363500 AND MessageType=2 AND (sendtime BETWEEN 0 AND 1672841368) AND  ((s_msgid2 BETWEEN 0 AND 4294967295) OR (s_msgid2 IS NULL) OR (s_msgid2=0)) AND UID=673837281 ORDER BY sendtime DESC  limit 502
            if(data_list[4] == "2"):
                sql1 = ''
                print(sql1)
                ret = db.selectone(sql1)
